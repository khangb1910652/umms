...
export const LayBinhLuanPbHdsdc = () => {
  return router.get(
    "/",
    async (
      req: express.Request,
      ...
      try {
        ...
        for (const element of PbHdsdcItems) {
          const TraLoiPbHdsdcItems = await knex("BINHLUAN")
          .select("*")
          .join('PHIENBANHDSD', 'PHIENBANHDSD.MAPHIENBAN', '=', 'BINHLUAN.MAPHIENBAN')
          .join(
              "NGUOIDUNG",
          ...
          element.TraLoiBL = TraLoiPbHdsdcItems;
        }

        // const PhItems = await knex('BINHLUAN')
        //     .select('BINHLUAN.THOIGIANBINHLUAN', 'BINHLUAN.MADV', 'BINHLUAN.MAPHCN', 'BINHLUAN.MACHUCNANG', 'BINHLUAN.MAHDSD', 'BINHLUAN.MAPHIENBAN', 'PHANHECHUCNANG.MAPHCN', 'PHANHECHUCNANG.TENPHCN', 'DICHVUSO.MADV', 'DICHVUSO.TENDV')
